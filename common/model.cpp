...
{
    Texture texture_obj; // Renamed from 'texture' to avoid conflict with common/texture.hpp potentially
    texture_obj.id = ::loadTexture(path); // Call the global loadTexture
    if (texture_obj.id == 0) { // Check for loading failure
        std::cerr << "Model failed to add texture: " << path << std::endl;
        // Optionally, don't add it to the textures vector or handle error
        return;
    }
    texture_obj.type = type;
    textures.push_back(texture_obj);
}
